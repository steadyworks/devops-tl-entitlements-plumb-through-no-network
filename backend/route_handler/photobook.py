...
class PhotobooksFullPublicResponse(PhotobooksOverviewResponse):
    ...
    async def rendered_from_dao(
        ...
        comment_daos: list[DAOPhotobookComments] = await DALPhotobookComments.list_all(
            db_session,
            filters={
                "photobook_id": (FilterOp.EQ, dao.id),
        ...
        comments = await PhotobookCommentsOverviewResponse.rendered_from_daos(
            comment_daos, db_session
        )
...
class PhotobookDetailsAuthorizedResponse(BaseModel):
    """
    Authenticated details exposed to user depending on user permission status.
    For example, only the photobook author can see the comments.
    """

    id: UUID
    comments: Optional[list[PhotobookCommentsOverviewResponse]] = None
...
class PhotobookAPIHandler(RouteHandler):
    def register_routes(self) -> None:
        ...
        self.route(
            "/api/photobook/{photobook_id}/details_authorized",
            "photobook_get_details_authorized",
            ["GET"],
        )
        self.route(
    ...
    async def photobook_create_comment(
        ...
        async with self.app.new_db_session() as db_session:
            async with safe_commit(db_session):
                ...
                return (
                    await PhotobookCommentsOverviewResponse.rendered_from_daos(
                        [comment], db_session=db_session
                    )
                )[0]

    @enforce_response_model
    @posthog_capture()
    async def photobook_get_details_authorized(
        self,
        request: Request,
        photobook_id: UUID,
    ) -> PhotobookDetailsAuthorizedResponse:
        request_context: RequestContext = await self.get_request_context(request)
        ...
        async with self.app.new_db_session() as db_session:
            ...
            if photobook is None:
                raise HTTPException(status_code=404, detail="Photobook not found")
            if logged_in_user_id != photobook.user_id:
                return PhotobookDetailsAuthorizedResponse(
                    id=photobook_id,
                    comments=None,
                )
    ...
    async def photobook_edit_comment(
        ...
        async with self.app.new_db_session() as db_session:
            ...
            async with safe_commit(db_session):
                ...
                return (
                    await PhotobookCommentsOverviewResponse.rendered_from_daos(
                        [updated_comment], db_session=db_session
                    )
                )[0]
